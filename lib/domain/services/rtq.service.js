...
let RtqService = class RtqService {
    ...
    async getDefaultGqlBaseApiPath() {
        const defaultGqlApiPath = await this._fileService.findFile(`**/data/rtq/${ConstantsHelper.DEFAULT_GQL_BASEAPI_NAME}`, {
    ...
    async getDefaultOpenapiBaseApiPath() {
        const defaultOpenapiPath = await this._fileService.findFile(`**/data/rtq/${ConstantsHelper.DEFAULT_OPENAPI_BASEAPI_NAME}`, {
            cwd: this._fileService.getLibDirname(),
        });
