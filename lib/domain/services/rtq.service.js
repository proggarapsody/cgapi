...
let RtqService = class RtqService {
    ...
    async getDefaultGqlBaseApiPath() {
        const defaultGqlApiPath = await this._fileService.findFile(`**/rtq/${ConstantsHelper.DEFAULT_GQL_BASEAPI_NAME}`, {
            rule: 'lib'
        });
    ...
    async getDefaultOpenapiBaseApiPath() {
        const defaultOpenapiPath = await this._fileService.findFile(`**/rtq/${ConstantsHelper.DEFAULT_OPENAPI_BASEAPI_NAME}`, {
            rule: 'lib',
        });
